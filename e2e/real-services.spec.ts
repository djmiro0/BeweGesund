...
function env(name: string) {
  ...
  if (!value) throw new Error(`${name} is required for real-service E2E tests.`);
  return value;
}
...
async function callFunction<T>(
  ...
  expect(response.ok, payload.error?.message ?? `${functionName} failed`).toBe(true);
  expect(payload.result, `${functionName} returned no callable result`).toBeTruthy();

  return payload.result as T;
}

test.describe("real service smoke tests", () => {
  test.skip(!runRealServiceE2e, "Set RUN_REAL_SERVICE_E2E=true and REAL_E2E_* env vars to hit real services.");

  test("creates a real Stripe Checkout Session without completing payment", async () => {
    const result = await callFunction<{ ok?: boolean; sessionId?: string; url?: string }>(
  ...
  test("gets a signed Mux playback token through the app API", async ({ request, baseURL }) => {
    const response = await request.post(`${baseURL ?? ""}/api/mux/playback-token`, {
      headers: {
    ...
    const payload = await response.json().catch(() => ({})) as { playbackToken?: string; code?: string; error?: string };
