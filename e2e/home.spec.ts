...
test.describe("pre-launch website", () => {
  test("keeps the public homepage behind the launch screen", async ({ page }) => {
    await page.goto("/de");

    await expect(page.getByRole("heading", { name: /website in vorbereitung/i })).toBeVisible();
  ...
  test("keeps direct public routes behind the same launch screen", async ({ page }) => {
