...
const appBaseURL = process.env.PLAYWRIGHT_BASE_URL
  ?? (process.env.PLAYWRIGHT_USE_EXISTING_SERVER === "true" ? "http://localhost:3000" : "http://127.0.0.1:3100");
...
async function installAuthOnboardingMocks(page: Page, events: EventName[]) {
  await page.exposeFunction("__recordBewegesundE2eEvent", (event: EventName) => {
    events.push(event);
  });
  await page.addInitScript(() => {
    ...
    e2eWindow.__BEWEGESUND_E2E_AUTH_MOCK__ = {
      saveProfile: async (path, payload) => {
        ...
        const profile = payload as { email?: string; anamnesis?: { goals?: string[] }; anamnesisStatusKey?: string };

        if (profile.email !== "e2e.member@example.com") {
          throw new Error(`Unexpected profile email: ${profile.email ?? "missing"}`);
        }

        if (!profile.anamnesis?.goals?.includes("muscle-fitness")) {
          throw new Error("Anamnesis payload did not include the selected goal.");
        }

        if (profile.anamnesisStatusKey !== "completed") {
          throw new Error(`Unexpected anamnesis status: ${profile.anamnesisStatusKey ?? "missing"}`);
        }
  ...
  await page.route("https://identitytoolkit.googleapis.com/v1/accounts:update?*", async (route) => {
  ...
  await page.route("**/createStripeCheckoutSession**", async (route) => {
    ...
    const requestBody = await readPostJson(route);
    expect(requestBody.data).toMatchObject({ locale: "de", memberPackage: "basic" });
  ...
  await page.route("**/confirmStripeCheckoutSession**", async (route) => {
    ...
    const requestBody = await readPostJson(route);
    expect(requestBody.data).toMatchObject({ sessionId: "cs_test_bewegesund_e2e" });
    await json(route, { result: { ok: true } });
  });

  await page.route("https://checkout.stripe.test/session/cs_test_bewegesund_e2e", async (route) => {
...
async function completeAccountStep(page: Page) {
  ...
  await page.getByPlaceholder("Passwort", { exact: true }).fill(firebaseUser.password);
  await page.getByPlaceholder("Passwort bestätigen", { exact: true }).fill(firebaseUser.password);
...
test.describe("account onboarding and payment", () => {
  test("creates an account, completes anamnesis, opens checkout, returns home, and signs in", async ({ browser, page }) => {
    ...
    await expect.poll(() => events).toContain("functions:createStripeCheckoutSession");
    await expect(page.getByRole("heading", { name: /stripe test checkout/i })).toBeVisible();
    ...
    await returningPage.locator("form").getByRole("button", { name: /^anmelden$/i }).click();
