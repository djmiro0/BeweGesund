...
export async function getFirebaseUserAccess(uid: string, idToken: string): Promise<FirebaseUserAccess> {
  ...
  if (!response.ok) {
    throw new Error("Firebase user profile could not be loaded.");
  }
