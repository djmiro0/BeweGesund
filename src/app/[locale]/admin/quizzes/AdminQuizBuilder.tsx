...
const copy = {
  de: {
    ...
    description:
      "Erstelle Fragen, markiere die richtige Antwort und speichere den Quiz direkt in Firestore.",
    signIn: "Bitte einloggen. Nur Admins können speichern.",
    adminRequired: "Admin-Zugriff erforderlich",
    setupDescription:
      "Wenn noch kein Admin existiert, kannst du den ersten Admin mit dem Setup-Code aktivieren.",
    setupCode: "Setup-Code",
    claimAdmin: "Admin aktivieren",
    claimingAdmin: "Wird aktiviert...",
    adminClaimed: "Admin-Zugriff aktiviert.",
    adminClaimError:
      "Admin konnte nicht aktiviert werden. Prüfe den Code oder ob bereits ein Admin existiert.",
    quizId: "Quiz ID",
    quizTitle: "Titel",
    ...
    error:
      "Quiz konnte nicht gespeichert werden. Prüfe Admin-Rolle und Pflichtfelder.",
  },
  en: {
    eyebrow: "Admin",
    title: "Create quiz",
    description:
      "Create questions, mark the correct answer, and save the quiz directly to Firestore.",
    signIn: "Please sign in. Only admins can save.",
    adminRequired: "Admin access required",
    setupDescription:
      "If no admin exists yet, you can activate the first admin with the setup code.",
    setupCode: "Setup code",
    claimAdmin: "Activate admin",
    claimingAdmin: "Activating...",
    adminClaimed: "Admin access activated.",
    adminClaimError:
      "Admin could not be activated. Check the code or whether an admin already exists.",
    quizId: "Quiz ID",
    quizTitle: "Title",
...
export default function AdminQuizBuilder({ locale }: { locale: string }) {
  ...
  const [title, setTitle] = useState(
    locale === "de" ? "Monatlicher Fitness-Quiz" : "Monthly Fitness Quiz",
  );
  const [description, setDescription] = useState("");
  const [quizLocale, setQuizLocale] = useState<"de" | "en">(
    locale === "en" ? "en" : "de",
  );
      ...
      current.map((question, currentQuestionIndex) =>
        currentQuestionIndex === questionIndex
          ? {
              ...
              options: [
                ...question.options,
                makeOption(question.options.length),
              ],
            }
          : question,
      ),
          ...
          <section className={styles.formGrid}>
            ...
            <label>
              ...
              <select
                value={quizLocale}
                onChange={(event) =>
                  setQuizLocale(event.target.value === "en" ? "en" : "de")
                }
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setPointsPerCorrect(Number(event.target.value))
                }
              />
            </label>
            <label>
              ...
              <input
                ...
                value={speedBonusMax}
                onChange={(event) =>
                  setSpeedBonusMax(Number(event.target.value))
                }
              />
            </label>
            <label>
            ...
            {questions.map((question, questionIndex) => (
              <article key={questionIndex} className={styles.questionCard}>
                <div className={styles.questionHeader}>
                  <h2>
                    {labels.question} {questionIndex + 1}
                  </h2>
                  {questions.length > 1 ? (
                    <button
                      type="button"
                ...
                <label>
                  ...
                  <input
                    value={question.id}
                    onChange={(event) =>
                      updateQuestion(questionIndex, { id: event.target.value })
                    }
                  />
                </label>
                <label>
