...
function getMusicTrackSource(musicLabel: string) {
  ...
  if (/rainforest|regenwald|forest|wald/.test(label))
    return "/music/rainforest.mp3";
  if (/ocean|ozean|wave|welle/.test(label))
    return "/music/ocean-wave-ambient.mp3";
...
function createSoftPad(index: number, musicEnabled = true, musicLabel = "") {
  ...
  let trackAudio = trackSource
    ? new Audio(new URL(trackSource, window.location.origin).toString())
    : null;
  ...
  const usesNoiseBed =
    /rain|regen|ocean|welle|wave|wald|forest|wind|natur|nature|cricket|grille|white noise|rauschen/.test(
      lowerMusicLabel,
    );
  ...
  overtone.type =
    lowerMusicLabel.includes("piano") || lowerMusicLabel.includes("klavier")
      ? "triangle"
      : "sine";
  ...
  const setFallbackMusicGain = (enabled: boolean) => {
    gain.gain.setTargetAtTime(
      enabled ? FALLBACK_MUSIC_VOLUME : 0,
  ...
  if (noiseSource && noiseFilter) {
    ...
    noiseFilter.type =
      lowerMusicLabel.includes("rain") ||
      lowerMusicLabel.includes("regen") ||
      lowerMusicLabel.includes("rauschen")
        ? "highpass"
        : "lowpass";
    noiseFilter.frequency.value =
      lowerMusicLabel.includes("rain") || lowerMusicLabel.includes("regen")
        ? 1550
  ...
  if (trackAudio) {
    ...
    trackAudio.addEventListener(
      "error",
      () => useFallbackMusic(currentMusicEnabled),
      { once: true },
    );
    trackAudio.load();
...
export default function BreathingTechniques({
  ...
  const toggleSession = () => {
    ...
    if (soundRef.current) {
      ...
      soundRef.current = createSoftPad(
        activeIndex,
        isMusicEnabled,
        activeSection?.music[selectedMusicIndex] ?? "",
      );
    }
  ...
  const selectMusic = (musicIndex: number) => {
    ...
    soundRef.current = createSoftPad(
      activeIndex,
      isMusicEnabled,
      activeSection.music[musicIndex] ?? "",
    );
  };
  ...
  const activeSection =
    activeIndex === null ? null : copy.sections[activeIndex];
  const activeVisualIndex = activeIndex ?? 0;
  const activePattern =
    activeIndex === null
      ? breathingPatterns[0]
      : (breathingPatterns[activeIndex] ?? breathingPatterns[0]);
  const activePhase: BreathPhase =
    activePattern[phaseIndex] ?? activePattern[0];
  const phaseLabel =
    activePhase.key === "inhale"
      ? copy.inhale
  ...
  return (
    <section className={styles.breathingSection} aria-label={copy.title}>
        ...
        <div className={styles.breathingPairList}>
          {copy.sections.map((section, index) => {
            ...
            return (
              <article
                key={section.title}
                className={`${styles.breathingCard} ${isActive ? styles.breathingCardActive : ""}`}
              >
      ...
      {infoIndex !== null && copy.education.items[infoIndex] ? (
        <div
          className={styles.breathingInfoPopupBackdrop}
        ...
        >
          ...
          >
            <button
              type="button"
              className={styles.breathingInfoPopupClose}
              onClick={() => setInfoIndex(null)}
            ...
            <h3 id="breathing-info-popup-title">
              {copy.education.items[infoIndex].title}
            </h3>
            <p>
              <strong>{copy.education.items[infoIndex].basis}</strong>
            </p>
      ...
      {activeSection ? (
        ...
        >
          <button
            type="button"
            className={styles.breathingSessionClose}
            onClick={close}
            aria-label={copy.close}
          >
            <X size={19} />
          ...
          <div className={styles.breathingOrbPanel}>
                ...
                style={
                  {
                    ...
                    "--breath-play-state": isSessionRunning
                      ? "running"
                      : "paused",
                  } as CSSProperties
                }
            ...
            <p className={styles.breathingSessionElapsed}>
              {formatSessionTime(elapsedSessionSeconds)}
            </p>
            <div
              className={styles.breathingAudioToggles}
              aria-label={copy.musicLabel}
            >
              <button
                type="button"
                ...
                onClick={toggleMusic}
                aria-label={
                  isMusicEnabled ? copy.musicOnLabel : copy.musicOffLabel
                }
              ...
              <button
                ...
                aria-label={
                  isBreathCueEnabled
                    ? copy.breathSoundOnLabel
                    : copy.breathSoundOffLabel
                }
                title={
                  isBreathCueEnabled
                    ? copy.breathSoundOnLabel
            ...
            <div
              className={styles.breathingMusicChoices}
              aria-label={copy.musicLabel}
            >
              {activeSection.music.map((music, musicIndex) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={
                      isSelected ? styles.breathingMusicChoiceActive : ""
                    }
          ...
          <div className={styles.breathingSessionDetails}>
            <div className={styles.breathingSessionMetaGrid}>
              <section>
                ...
                <ul>
                  {activeSection.rhythm.map((item, itemIndex) => (
            ...
            {activeSection.note ? (
              <p className={styles.musicCue}>{activeSection.note}</p>
            ) : null}
          </div>
        </div>
      ) : null}
