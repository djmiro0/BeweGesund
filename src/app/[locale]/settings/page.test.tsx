...
describe("SettingsPage", () => {
  beforeEach(() => {
    firebaseMocks.getDownloadURL
      .mockReset()
      .mockResolvedValue("https://storage.example/avatar?token=test");
    firebaseMocks.getBlob
      .mockReset()
      .mockResolvedValue(new Blob(["preview"], { type: "image/jpeg" }));
    firebaseMocks.sendPasswordResetEmail
      .mockReset()
  ...
  it("renders profile and gamification values from Firebase", async () => {
    ...
    expect(
      await screen.findByDisplayValue("Firebase Profile Name"),
    ).toBeInTheDocument();
    ...
    expect(screen.getByTestId("settings-badges-placeholder")).toHaveTextContent(
      "first-workout",
    );
  });
  ...
  it("saves profile and preference changes to Firebase", async () => {
    ...
    expect(screen.getByTestId("settings-dirty-message")).toHaveTextContent(
  ...
  it("sends a password reset email from account settings", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Send reset email" }),
    );
    ...
    expect(
      screen.getByText("A password-change email was sent."),
    ).toBeInTheDocument();
  });
    ...
    expect(firebaseMocks.updateDoc).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        photoURL: expect.stringContaining(
          "https://storage.example/avatar?token=test&v=",
        ),
      }),
    );
  ...
  it("uploads a private before photo and saves its status", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: /Before & After/ }),
    );
    await user.upload(
      await screen.findByLabelText("sections.progressPhotos.beforeAction"),
      file,
    );
  ...
  it("opens account deletion from the bottom of settings", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Delete profile" }),
    ).toBeInTheDocument();
