...
export default function LiveSchedule() {
  ...
  return (
    <section
      id="live"
      className="py-20 px-6 bg-[rgba(var(--navy-rgb),0.62)] border-y border-[var(--border-soft)]"
    >
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
          <h2 className="text-4xl md:text-6xl font-black uppercase italic text-[var(--text-light)]">
            {t("title")}{" "}
            <span className="text-transparent bg-clip-text bg-[linear-gradient(90deg,var(--highlight),var(--highlight-strong))]">
              {t("highlight")}
            </span>
          </h2>
          <Link
            href={`/${locale}/calendar`}
            className="text-[var(--highlight)] font-bold uppercase text-sm hover:underline"
          >
            {t("viewAll")} →
          </Link>
        </div>
        ...
        <div className="space-y-4">
          {items.map((item) => (
            <div
              ...
              className="flex flex-col md:flex-row items-center bg-[var(--surface-overlay)] border border-[var(--border-soft)] p-6 rounded-2xl hover:border-[var(--border-strong)] transition group"
            >
              {/* Date Box */}
              <div className="flex flex-row md:flex-col items-center gap-2 md:gap-0 md:bg-[rgba(var(--foreground-rgb),0.06)] md:p-4 rounded-xl md:w-24 text-center mb-4 md:mb-0 mr-0 md:mr-8 border border-[rgba(var(--foreground-rgb),0.06)] md:border-transparent">
                <span className="text-[var(--highlight)] font-black uppercase text-sm md:text-xs tracking-wider">
                  {item.day}
                </span>
                <span className="text-[var(--text-light)] font-bold text-lg leading-none">
                  {item.date}
                </span>
              </div>
