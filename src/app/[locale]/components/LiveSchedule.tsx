            ...
            <div className="max-w-5xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                    <h2 className="text-4xl md:text-6xl font-black uppercase italic text-white">
                        Live <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-600">Kurse</span>
                    </h2>
                    <button className="text-orange-500 font-bold uppercase text-sm hover:underline">Alle Termine ansehen →</button>
                </div>

                <div className="space-y-4">
                    {mockSchedule.map((item) => (
                        <div key={item.id} className="flex flex-col md:flex-row items-center bg-black border border-white/10 p-6 rounded-2xl hover:border-orange-500 transition group">
                            {/* Date Box */}
                            <div className="flex flex-row md:flex-col items-center gap-2 md:gap-0 md:bg-zinc-900 md:p-4 rounded-xl md:w-24 text-center mb-4 md:mb-0 mr-0 md:mr-8 border border-white/5 md:border-transparent">
                                ...
                                <span className="text-white font-bold text-lg leading-none">{item.date.split('.')[0]}</span>
                            </div>
                            ...
                            <div className="flex-1 text-center md:text-left">
                                ...
                                <div className="flex justify-center md:justify-start gap-6 text-zinc-400 text-sm font-medium">
                                    ...
                                    <span className="flex items-center gap-2"><Users size={16} className="text-orange-600"/> {item.spots} Plätze frei</span>
                                </div>
                            </div>
                            ...
                            <button className="mt-4 md:mt-0 px-6 py-3 bg-white text-black font-black uppercase text-sm rounded hover:bg-orange-500 hover:text-white transition transform skew-x-[-10deg]">
                                <span className="inline-block skew-x-[10deg]">Buchen</span>
                            </button>
                        </div>
                    ))}
                </div>
