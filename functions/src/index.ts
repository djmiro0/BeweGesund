...
function calculateRequiredSeconds(
  config: ContentRewardConfig,
  ...
  if (
    ...
    return Math.max(
      1,
      Math.ceil(durationSeconds * config.requiredDurationRatio),
    );
  }

  throw new HttpsError(
    "invalid-argument",
    "durationSeconds is required for this content type.",
  );
}
...
function getRewardState(
  progress: Pick<
    ...
    "completed" | "rewardAvailable" | "rewardClaimed"
  >,
...
function buildUserPointIncrements(points: number) {
  return MONTHLY_POINTS_FIELDS.reduce<
...
async function claimContentRewardForUser(
  ...
  const dailyRewardRef = profileRef
    .collection(DAILY_REWARDS_COLLECTION)
    .doc(today);
  const ledgerRef = profileRef
  ...
  if (config.rewardPolicy === "LIMITED_PER_DAY")
    reads.push(transaction.get(dailyRewardRef));
  ...
  if (!progressSnapshot.exists) {
    throw new HttpsError(
      "failed-precondition",
      "Content progress does not exist.",
    );
  }
  ...
  const engagementSeconds = Math.max(
    0,
    Math.floor(progress.engagementSeconds ?? 0),
  );
  const requiredSeconds = Math.max(
    0,
    Math.floor(progress.requiredSeconds ?? 0),
  );
  ...
  if (
    ...
    throw new HttpsError(
      "failed-precondition",
      "Content progress does not match this reward.",
    );
  }
  ...
  updateRegionalLeaderboardEntries(
    transaction,
    profileSnapshot,
    uid,
    config.points,
  ...
  if (config.rewardPolicy === "LIMITED_PER_DAY") {
    transaction.set(
      dailyRewardRef,
      {
        ...
        rewardedContentIds: Array.from(
          new Set([...rewardedContentIds, contentId]),
        ),
        updatedAt: serverTimestamp(),
        createdAt: serverTimestamp(),
      },
...
function normalizeSaveQuizPayload(payload: SaveQuizPayload) {
  ...
  const normalizedQuestions = questions.map((question, questionIndex) => {
    const questionId = requireStringId(
    ...
    if (options.length < 2 || options.length > 8) {
      throw new HttpsError(
        "invalid-argument",
        "Each question needs 2 to 8 answers.",
      );
    }
    ...
    const correctOptionId = requireStringId(
  ...
  return {
    ...
    publicQuiz: {
      ...
      availableUntil,
      monthlyPeriod:
        typeof payload.monthlyPeriod === "string"
          ? payload.monthlyPeriod.trim()
          : currentMonthPeriod(),
      timeLimitSeconds: Number.isFinite(payload.timeLimitSeconds)
        ? Math.max(
            MIN_QUIZ_QUESTION_SECONDS,
            Math.round(payload.timeLimitSeconds ?? MIN_QUIZ_QUESTION_SECONDS),
          )
        : MIN_QUIZ_QUESTION_SECONDS,
      allowRetake: payload.allowRetake === true,
      pointsPerCorrect: Number.isFinite(payload.pointsPerCorrect)
        ? Math.max(1, Math.round(payload.pointsPerCorrect ?? 100))
        : 100,
      speedBonusMax: Number.isFinite(payload.speedBonusMax)
        ? Math.max(0, Math.round(payload.speedBonusMax ?? 50))
        : 50,
      xpReward: Number.isFinite(payload.xpReward)
        ? Math.max(0, Math.round(payload.xpReward ?? 10))
        : 10,
...
function requireRecentSignIn(authTime: unknown) {
  const authTimeSeconds =
    typeof authTime === "number" ? authTime : Number(authTime);
  const fiveMinutesInSeconds = 5 * 60;

  if (
    !Number.isFinite(authTimeSeconds) ||
    Date.now() / 1000 - authTimeSeconds > fiveMinutesInSeconds
  ) {
    throw new HttpsError(
      "failed-precondition",
      "Recent sign-in is required before deleting the account.",
    );
  }
}
...
function getStripePriceId(memberPackage: MemberPackage) {
  return memberPackage === "plus"
    ? stripePlusPriceId.value()
    : stripeBasicPriceId.value();
}

function getMemberPackageFromMetadata(
  value: unknown,
...
function getInternalSubscriptionStatus(
...
function subscriptionAccess(
  ...
  const metadataPackage =
    getMemberPackageFromMetadata(subscription.metadata.memberPackage) ??
    fallbackPackage;

  return {
    memberPackage: getMemberPackageForPrice(
...
async function applyCompletion(
  ...
  const completionCollection =
    kind === "lesson" ? "lessonCompletions" : "workoutCompletions";
...
export const updateContentEngagement = onCall(
  ...
  async (request) => {
    ...
    const engagementDelta =
      typeof payload.engagementSeconds === "number" &&
      Number.isFinite(payload.engagementSeconds)
        ? Math.max(
            0,
            Math.min(
              MAX_ENGAGEMENT_SECONDS_PER_UPDATE,
              Math.floor(payload.engagementSeconds),
            ),
          )
        : 0;
    ...
    if (engagementDelta <= 0 && requiredSeconds > 0) {
      throw new HttpsError(
        "invalid-argument",
        "engagementSeconds must be greater than 0.",
      );
    }
    ...
    return db.runTransaction(async (transaction) => {
      ...
      const existing = progressSnapshot.exists
        ? (progressSnapshot.data() as ContentProgressDocument)
        : null;

      if (existing?.contentType && existing.contentType !== contentType) {
        throw new HttpsError(
          "failed-precondition",
          "Content progress exists for a different content type.",
        );
      }

      const currentEngagementSeconds = Math.max(
        0,
        Math.floor(existing?.engagementSeconds ?? 0),
      );
      const nextEngagementSeconds = existing?.rewardClaimed
        ? currentEngagementSeconds
        : Math.max(
            currentEngagementSeconds,
            currentEngagementSeconds + engagementDelta,
          );
      const completed = nextEngagementSeconds >= requiredSeconds;
      const completedAt =
        existing?.completedAt ?? (completed ? serverTimestamp() : null);
      ...
      transaction.set(
        progressRef,
        {
          ...
          dailyLimitReached: false,
          completedAt,
          createdAt: progressSnapshot.exists
...
export const claimContentReward = onCall(
  ...
  async (request) => {
    ...
    return db.runTransaction((transaction) =>
      claimContentRewardForUser(uid, contentId, contentType, transaction),
    );
  },
);
...
export const deleteUserAccount = onCall(
  ...
  async (request) => {
    ...
    const subscriptionId = profileSnapshot.get("stripeSubscriptionId") as
      string | undefined;
...
export const claimFirstAdmin = onCall(
  ...
  async (request) => {
    const uid = requireAuth(request.auth);
    const setupCode =
      typeof request.data?.setupCode === "string"
        ? request.data.setupCode.trim()
        : "";

    if (!setupCode || setupCode !== adminSetupCode.value()) {
      throw new HttpsError(
        "permission-denied",
        "The admin setup code is invalid.",
      );
    }
...
export const saveQuiz = onCall(appCheckCallableOptions, async (request) => {
  ...
  const { quizId, publicQuiz, answerKey } = normalizeSaveQuizPayload(
    request.data as SaveQuizPayload,
  );
  ...
  await Promise.all([
    db
      .collection(QUIZZES_COLLECTION)
      .doc(quizId)
      .set(publicQuiz, { merge: true }),
    db
      .collection(QUIZ_ANSWER_KEYS_COLLECTION)
      .doc(quizId)
      .set(answerKey, { merge: true }),
  ]);
...
export const checkQuizAnswer = onCall(
  ...
  async (request) => {
    ...
    if (
      ...
      answeredAt.getTime() > Date.now() + 60_000
    ) {
    ...
    const answerKey = answerKeySnapshot.data() as
    ...
    return db.runTransaction(async (transaction) => {
      ...
      if (attemptSnapshot.exists && !quiz.allowRetake) {
        ...
        return {
          ...
          answers: Array.isArray(previousAttempt.answers)
      ...
      const correctAnswers = Object.entries(answerKey.answers).filter(
        ([questionId, optionId]) =>
          submittedQuestionIds.has(questionId) && typeof optionId === "string",
      );
      ...
      let correctCount = 0;
      const scoredAnswers = correctAnswers.map(
      ...
      const pointsPerCorrect = Number.isFinite(quiz.pointsPerCorrect)
        ? Math.max(1, quiz.pointsPerCorrect ?? 100)
        : 100;
      const speedBonusMax = Number.isFinite(quiz.speedBonusMax)
        ? Math.max(0, quiz.speedBonusMax ?? 50)
        : 50;
      const secondsPerQuestion =
        Number.isFinite(quiz.timeLimitSeconds) && quiz.timeLimitSeconds
      ...
      const speedRatio =
        durationMs !== null
          ...
          : 0;
      const speedBonus =
        correctCount > 0 ? Math.round(speedBonusMax * speedRatio) : 0;
      const score = correctCount * pointsPerCorrect + speedBonus;
      const xpGain = Number.isFinite(quiz.xpReward)
        ? Math.max(0, quiz.xpReward ?? 10)
        : 10;
...
export const updateStreak = onCall(appCheckCallableOptions, async (request) => {
  ...
  previous.setHours(0, 0, 0, 0);
  const diffDays = Math.floor(
    (today.getTime() - previous.getTime()) / 86400000,
  );
...
async function rebuildLeaderboard(period: "weekly" | "monthly") {
  ...
  const rankField =
    period === "weekly" ? "weeklyLeaderboardRank" : "monthlyLeaderboardRank";
  const snapshot = await db
    .collection("users")
...
export const claimReward = onCall(appCheckCallableOptions, async (request) => {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const rewardData = rewardSnapshot.data() as {
      cost?: number;
      active?: boolean;
    };
...
export const createStripeCheckoutSession = onCall(
  ...
  async (request) => {
    ...
    if (memberPackage !== "basic" && memberPackage !== "plus") {
      throw new HttpsError(
        "invalid-argument",
        "memberPackage must be basic or plus.",
      );
    }
    ...
    if (!customerId) {
      const customer = await stripe.customers.create({
        email:
          (request.auth?.token.email as string | undefined) ?? profile?.email,
        metadata: { uid },
      });
      customerId = customer.id;
      await profileRef.set(
        { stripeCustomerId: customerId, updatedAt: serverTimestamp() },
        { merge: true },
      );
    }
...
export const confirmStripeCheckoutSession = onCall(
  ...
  async (request) => {
    ...
    const customerId = profileSnapshot.get("stripeCustomerId") as
      string | undefined;
    const stripe = createStripeClient();
    ...
    if (session.client_reference_id !== uid && session.metadata?.uid !== uid) {
      throw new HttpsError(
        "permission-denied",
        "The checkout session does not belong to this user.",
      );
    }
    ...
    if (customerId && sessionCustomerId !== customerId) {
      throw new HttpsError(
        "permission-denied",
        "The checkout customer does not match this user.",
      );
    }
...
export const createGoogleHealthAuthorizationUrl = onCall(
  ...
  async (request) => {
    ...
    await db
      .collection("googleHealthOAuthStates")
      .doc(state)
      .set({
        uid,
        locale,
...
async function exchangeGoogleHealthCode(code: string) {
  ...
  if (!response.ok) {
    throw new Error(
      `Google Health token exchange failed with status ${response.status}.`,
    );
  }
...
async function refreshGoogleHealthToken(uid: string, refreshToken: string) {
  ...
  if (!response.ok) {
    throw new HttpsError(
      "failed-precondition",
      "Google Health authorization has expired. Reconnect wearable.",
    );
  }
...
async function getGoogleHealthAccessToken(uid: string) {
  const connectionRef = userRef(uid)
    .collection("wearables")
    .doc(GOOGLE_HEALTH_PROVIDER);
  ...
  if (!data.accessToken || !data.refreshToken) {
    throw new HttpsError(
      "failed-precondition",
      "Google Health connection is incomplete.",
    );
  }
...
async function fetchGoogleHealthJson<T>(
  ...
  if (!response.ok) {
    throw new HttpsError(
      "unavailable",
      `Google Health request failed with status ${response.status}.`,
    );
  }
...
async function syncGoogleHealthDailySummaryForUser(uid: string, date: string) {
  ...
  const activeMinutesSeconds = firstRollupNumber(
    activeMinutes,
    "activeMinutes",
    ["durationSumSeconds"],
  );
  const activeMinutesCount = firstRollupNumber(activeMinutes, "activeMinutes", [
    "minutesSum",
    "countSum",
    "count",
  ]);
  const sleepSeconds = firstRollupNumber(sleep, "sleep", [
    "durationSumSeconds",
  ]);
  const sleepMinutes = firstRollupNumber(sleep, "sleep", [
    "minutesAsleep",
  ...
  const dailySummary = {
    ...
    activeMinutes:
      secondsToRoundedMinutes(activeMinutesSeconds) ?? activeMinutesCount ?? 0,
    caloriesOut: null,
    restingHeartRate:
      firstRollupNumber(restingHeartRate, "dailyRestingHeartRate", [
        "beatsPerMinute",
        "bpm",
        "average",
      ]) ?? null,
...
export const googleHealthOAuthCallback = onRequest(
  ...
  async (request, response) => {
    ...
    if (!code || !stateSnapshot?.exists) {
      response
        .status(400)
        .send("Invalid Google Health authorization callback.");
      return;
    }
    ...
    if (
      ...
      stateData.expiresAtMs < Date.now()
    ) {
      await stateRef.delete().catch(() => undefined);
      response
        .status(400)
        .send("Expired Google Health authorization callback.");
      return;
    }
    ...
    try {
      ...
      const identity =
        await fetchGoogleHealthJson<GoogleHealthIdentityResponse>(
          token.access_token,
          "users/me/identity",
        );
      await userRef(stateData.uid)
        .collection("wearables")
        .doc(GOOGLE_HEALTH_PROVIDER)
        .set(
          {
      ...
      response.redirect(
        `${appBaseUrl.value().replace(/\/$/, "")}/${safeLocale}/profile?wearable=google-health-connected`,
      );
    } catch (error) {
...
export const syncGoogleHealthDailySummary = onCall(
  ...
  async (request) => {
    const uid = requireAuth(request.auth);
    const date =
      typeof request.data?.date === "string" &&
      /^\d{4}-\d{2}-\d{2}$/.test(request.data.date)
        ? request.data.date
        : todayIsoDate();
